#!/usr/bin/env python3
"""
K2Think API - Enhanced Windows Deployment Script
This script provides improved deployment for Windows systems with better error handling
"""
...
def print_banner():
    ...
    print("🚀 K2Think API - Enhanced Windows Deployment")
    print("=" * 60)
...
def get_or_create_credentials():
    """Get or create K2Think credentials"""
    ...
    if Path(accounts_file).exists():
        try:
            ...
            print(f"✅ Found existing credentials for: {credentials.get('email', 'unknown')}")
    ...
    import getpass
    password = getpass.getpass("Password [developer123]: ").strip()
    if not password:
...
def setup_environment():
    ...
    if result.returncode != 0:
        ...
        if result.stderr:
            print("📝 Error details (partial):", result.stderr[:200] + "..." if len(result.stderr) > 200 else result.stderr)
    else:
...
def main():
    ...
    try:
        # Step 1: Credentials
        email, password = get_or_create_credentials()
